(function() {
  const pixelArray = draw({
    width: 120,
    height: 40,
    padd: 4
  });

  displayMatrix('#canvas', pixelArray);
   ...
   * @param {number} param0.padd
   */
  function draw({ width, height, padd }) {
    const emptyMark = 0;
    ...
    const matrixArt = applyMatrixArt(matrix);
    return matrixArt;
  }
   ...
   * @returns {string}
   */
  function applyMatrixArt(matrixRef) {
    const pixels = [' ', '-', '|'];
    let drawing = '';
   ...
   */
  function displayMatrix(domSelector, matrixArt) {
